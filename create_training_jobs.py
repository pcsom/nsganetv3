...
def create_train_job(arch_id, config_path, arch_dir, supernet_path, dataset, data_path, 
                     epochs, batch_size, conda_env, time_limit='24:00:00'):
    
    img_size = 32 if 'cifar' in dataset.lower() else 224
...
if [ $TRAIN_EXIT_CODE -eq 0 ]; then
    ...
    mv *.log {arch_dir}/ 2>/dev/null || true
    mv *.txt {arch_dir}/ 2>/dev/null || true
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default=os.path.expanduser('~/scratch/datasets/cifar'))
    ...
    if not os.path.exists(metadata_path):
        ...
        print(f"Run generate_training_corpus.py first")
        return
    ...
    for item in corpus:
        ...
        job_file = create_train_job(
            ...
            args.dataset, args.data_path, args.epochs, args.batch_size,
